-- ================================================================
-- 3 reporting functions for AdminFinancials page
-- ================================================================
...
DROP FUNCTION IF EXISTS public.get_financial_summary_by_psychologist(TIMESTAMPTZ, TIMESTAMPTZ);

-- 1. Platform-wide summary card data
...
-- 2. Detailed transaction report (one row per recognized event)
...
BEGIN
  ...
  SELECT
    ...
    MAX(wt.psychologist_id)                                                          AS psychologist_id,
  ...
  -- Events WITHOUT appointment_id (PART B account deletions: remaining deferred to admin)
  SELECT
...
-- 3. Summary grouped by psychologist
CREATE OR REPLACE FUNCTION public.get_financial_summary_by_psychologist(
  _from_date TIMESTAMPTZ DEFAULT NULL,
  _to_date   TIMESTAMPTZ DEFAULT NULL
)
RETURNS TABLE(
...
BEGIN
  RETURN QUERY
  SELECT
    pp.id                                                                                   AS psychologist_id,
    pp.first_name || ' ' || pp.last_name                                                    AS psychologist_name,
    COUNT(DISTINCT CASE
      WHEN wt.transaction_type = 'session_completed' AND wt.wallet_type = 'psychologist'
      THEN wt.appointment_id
    END)::BIGINT                                                                             AS sessions_count,
    COALESCE(SUM(wt.amount), 0)                                                             AS gross_income,
    COALESCE(SUM(CASE WHEN wt.wallet_type = 'admin'        THEN wt.amount ELSE 0 END), 0)  AS admin_commission,
    COALESCE(SUM(CASE WHEN wt.wallet_type = 'psychologist' THEN wt.amount ELSE 0 END), 0)  AS psychologist_payment,
    COALESCE((
      SELECT SUM(dr.deferred_amount)
      FROM deferred_revenue dr
      WHERE dr.psychologist_id = pp.id
    ), 0)                                                                                    AS pending_deferred,
    COALESCE(SUM(CASE WHEN wt.transaction_type = 'account_deletion' THEN wt.amount ELSE 0 END), 0)
                                                                                             AS account_deletion_income
  FROM psychologist_profiles pp
  LEFT JOIN wallet_transactions wt
    ON wt.psychologist_id = pp.id
    AND wt.transaction_type IN ('session_completed', 'account_deletion')
    AND (_from_date IS NULL OR wt.created_at >= _from_date)
    AND (_to_date   IS NULL OR wt.created_at <= _to_date)
  GROUP BY pp.id, pp.first_name, pp.last_name
  HAVING
    COALESCE(SUM(wt.amount), 0) > 0
    OR COALESCE((
      SELECT SUM(dr.deferred_amount) FROM deferred_revenue dr WHERE dr.psychologist_id = pp.id
    ), 0) > 0
