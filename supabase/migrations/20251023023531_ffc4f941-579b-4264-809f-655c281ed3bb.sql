-- Create secure RPC to list therapist's patients with minimal profile info
create or replace function public.get_therapist_patients()
returns table (
  patient_id uuid,
  full_name text,
  avatar_url text,
  email text,
  session_count integer,
  last_session timestamptz
)
language sql
stable
security definer
set search_path = public
as $$
  select
    a.patient_id,
    p.full_name,
    p.avatar_url,
    p.email,
    count(*)::int as session_count,
    max(a.start_time) as last_session
  from public.appointments a
  join public.profiles p on p.id = a.patient_id
  where a.psychologist_id in (
    select id from public.psychologist_profiles
    where public.psychologist_profiles.user_id = auth.uid()
      and public.has_role(auth.uid(), 'psicologo')
  )
  group by a.patient_id, p.full_name, p.avatar_url, p.email
$$;

-- Optional: grant execute to authenticated users (Lovable Cloud handles this by default)
-- but we ensure it's available
revoke all on function public.get_therapist_patients from public;
grant execute on function public.get_therapist_patients to authenticated;