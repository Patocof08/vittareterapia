...
export default function AdminDashboard() {
  ...
  const recalculateHistoricalPayments = async () => {
    try {
      toast.info("Recalculando pagos de paquetes...");
      const { data: packagePayments } = await supabase
        .from("payments")
        .select(`
          id,
      ...
      const found = packagePayments?.length || 0;
      if (!packagePayments || packagePayments.length === 0) {
        toast.info("No hay pagos de paquetes para recalcular");
        return;
      }
      ...
      for (const payment of packagePayments) {
        ...
        // @ts-ignore - RPC types autogenerated
        const { error: rpcError } = await supabase.rpc('recalculate_package_financials', {
          _subscription_id: payment.subscription_id,
          _payment_id: payment.id,
          _psychologist_id: payment.psychologist_id,
          _total_amount: payment.amount,
        ...
        if (!rpcError) {
          ...
          console.error("Error recalculando pago:", payment.id, rpcError);
        }
      }

      if (processed > 0) {
        toast.success(`${processed}/${found} pago(s) recalculado(s)`);
        fetchFinancialStats();
        fetchStats();
      } else {
        toast.info(`0/${found} pagos recalculados`);
      }
    } catch (error) {
      console.error("Error recalculando pagos históricos:", error);
      toast.error("Error al recalcular pagos");
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="mt-4 flex flex-col md:flex-row md:items-center md:gap-3">
          ...
          <Button
            variant="secondary"
            onClick={recalculateHistoricalPayments}
            className="w-full md:w-auto mt-2 md:mt-0"
          >
            <RefreshCw className="w-4 h-4 mr-2" />
            Recalcular Pagos (Forzar)
          </Button>
          <p className="text-xs text-muted-foreground mt-2 md:mt-0">
            Usa "Recalcular" si ya existe ingreso diferido y quieres aplicar el nuevo esquema (descuentos absorbidos por admin).
          </p>
        </div>
      </div>
